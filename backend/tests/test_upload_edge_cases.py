def test_upload_chunk_larger_than_expected(
    client, auth_headers, clean_storage, clean_upload_manager
):
...
def test_upload_duplicate_chunk(client, auth_headers, clean_storage, clean_upload_manager):
    ...
    for i in range(2):
        ...
        # Idempotency: 200 (with "exists") or 204 (No Content) are both valid
        assert chunk_res.status_code in [200, 204]
        if i == 0:
            assert chunk_res.json()["status"] == "received"
        elif chunk_res.status_code == 200:
            assert chunk_res.json()["status"] == "exists"

    # Status should show only 1 chunk (not 2)
    status_res = client.get(
        f"/upload/{upload_id}/status", headers={"Authorization": f"Bearer {upload_token}"}
    )
...
def test_upload_complete_with_missing_chunks(
    client, auth_headers, clean_storage, clean_upload_manager
):
    ...
    if complete_res.status_code == 200:
        ...
        assert (
            result["status"] in ["incomplete", "error"]
