...
async def test_status_reports_missing_chunks(client, auth_headers):
    ...
    # Init
    init_data = {
        "study_metadata": {"patient_name": "Missing Chunks", "study_date": "2023", "modality": "OT"},
        "total_files": 1,
        "total_size_bytes": 1024
    }
    resp = client.post("/upload/init", json=init_data, headers=auth_headers)
    ...
    client.put(f"/upload/{upload_id}/chunk?chunk_index=0&file_id=file1", content=b"a"*512, headers=headers)
    ...
    client.put(f"/upload/{upload_id}/chunk?chunk_index=2&file_id=file2", content=b"b"*512, headers=headers)
