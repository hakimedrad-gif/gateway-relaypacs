...
describe('UploadManagerService', () => {
  ...
  it('should create a study and store files in IndexedDB', async () => {
    const mockFiles = [new File(['dummy context'], 'test1.dcm', { type: 'application/dicom' })];
    const mockMetadata = {
  ...
  it('should orchestrate the upload flow correctly', async () => {
    ...
    const mockFiles = [new File([blobContent], 'test.dcm', { type: 'application/dicom' })];
  ...
  it('should resume an incomplete upload by fetching status', async () => {
    ...
    const mockFiles = [new File([blobContent], 'test_resume.dcm', { type: 'application/dicom' })];
